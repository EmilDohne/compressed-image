...
NAMESPACE_COMPRESSED_IMAGE
{
    ...
    struct channel : public std::ranges::view_interface<channel<T>>
    {
        ...
        )
        {
            ...
            m_CompressionContext = blosc2::create_compression_context<T>(
                std::thread::hardware_concurrency() / 2, m_Codec, m_CompressionLevel, block_size);
            m_DecompressionContext = blosc2::create_decompression_context(std::thread::hardware_concurrency() / 2);
        ...
        )
        {
            ...
            // wrapper
            std::visit([&](auto& _schunk)
            {
                m_CompressionContext = blosc2::create_compression_context<T>(
                    std::thread::hardware_concurrency() / 2, m_Codec, m_CompressionLevel, schunk.max_block_size());
                m_DecompressionContext = blosc2::create_decompression_context(std::thread::hardware_concurrency() / 2);
            }, *m_Schunk);
        }
        ...
        {
            return channel<T>::full(width, height, static_cast<T>(0), compression_codec, compression_level, block_size,
                                    chunk_size);
        }
        ...
        iterator end()
        {
            if (m_Schunk)
            {
                ...
                {
                    return iterator(m_Schunk, m_CompressionContext.get(), m_DecompressionContext.get(),
                                    schunk.num_chunks(), m_Width, m_Height);
                }, *m_Schunk);
            }
        ...
        {
            m_CompressionContext = blosc2::create_compression_context<T>(
                nthreads, m_Codec, m_CompressionLevel, block_size);
