...
namespace NAMESPACE_COMPRESSED_IMAGE
{
	...
	template <typename T>
	struct schunk final: 
		public detail::schunk_mixin<T, cuda::compressed_chunk<T>, std::vector<std::byte>>
	{
		using detail::schunk_mixin<T, cuda::compressed_chunk<T>, std::vector<std::byte>>::chunk_bytes;
		using detail::schunk_mixin<T, cuda::compressed_chunk<T>, std::vector<std::byte>>::is_gpu_chunk;
		...
		void chunk(cuda::nvcomp_context decompression_ctx, std::span<T> buffer, size_t index) const override
		{
			...
			const auto& chunk_data = std::get<detail::schunk_mixin::gpu_container>(this->m_Chunks.at(index));
			auto compressor = cuda::make_compressor(chunk_data);
			compressor.decompress(chunk_data, std::span<T>(buffer));
		...
		void chunk(blosc2::context_raw_ptr decompression_ctx, std::span<T> buffer, size_t index) const override
		{
			...
			const auto& chunk_data = std::get<detail::schunk_mixin::cpu_container>(this->m_Chunks.at(index));
		...
		void set_chunk(cuda::nvcomp_context compression_ctx, std::span<T> uncompressed, size_t index) override
		{
			...
			auto compressor = cuda::make_compressor(compression_ctx.codec);
			auto _chunk = compressor.compress(compression_ctx, uncompressed);
		...
		size_t chunk_bytes(size_t index) const override
		{
			...
			{
				const auto& _chunk = std::get<detail::schunk_mixin::gpu_container>(this->m_Chunks.at(index));
				return _chunk.size();
			}
			const auto& _chunk = std::get<detail::schunk_mixin::cpu_container>(this->m_Chunks.at(index));
