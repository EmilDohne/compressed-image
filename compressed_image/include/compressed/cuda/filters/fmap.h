...
/// The fmap compiled plugin. We assume that this plugin lives somewhere on the search path and load it from there
/// Note:
///     If this is ever changed in the cmake, the plugin name must be updated here as well.
namespace
NAMESPACE_COMPRESSED_IMAGE::cuda::detail::fmap
{
#if defined(_WIN32)
    constexpr static std::string_view s_plugin_name = "compressed_fmap_plugin.dll";
#elif defined(__linux__)
    constexpr static std::string_view s_plugin_name = "libcompressed_fmap_plugin.so";
#else
...
NAMESPACE_COMPRESSED_IMAGE
{
    namespace cuda
    {
        namespace filter
        {
            /// \brief fmap filter plugin for CUDA compression.
            ///
            /// This filter is dynamically loaded and hooked as a plugin. It can only be loaded if the plugin
            /// `s_plugin_name` is found on the search path.
            ///
            /// It applies a lossless monotonic order map to IEEE-754 bit patterns (half/float/double via
            /// type_size 2/4/8) so that unsigned integer ordering matches numeric ordering. Intended to run
            /// FIRST in a pipeline, ahead of delta, making arithmetic deltas on float data well-behaved
            /// across sign changes and magnitude boundaries. NaN, +/-Inf, -0.0 and denormals round-trip
            /// bit-exactly.
            struct fmap
            {
                constexpr static cuda::enums::filter filter_type()
                {
                    return cuda::enums::filter::fmap;
                }

                static fmap& instance()
                {
                    static fmap inst;
                    return inst;
                }
            ...
            template <typename Func, typename... Args>
            void fmap::plugin_call(Func func, std::string_view func_name, Args&&... args)
            {
                if (!func)
                {
            ...
            inline fmap::fmap()
            {
                handle_ = proc::load_library(std::string(detail::fmap::s_plugin_name));
                if (!handle_)
                {
                    return;
                }
                ...
                this->forward_fn_ = proc::get_symbol<decltype(this->forward_fn_)>(
                    handle_,
                    "run_fmap_forward",
                    std::string(detail::fmap::s_plugin_name)
                );
