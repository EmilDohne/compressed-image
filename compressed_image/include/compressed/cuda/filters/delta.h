...
/// The delta compiled plugin. We assume that this plugin lives somewhere on the search path and load it from there
/// Note:
///     If this is ever changed in the cmake, the plugin name must be updated here as well.
namespace
NAMESPACE_COMPRESSED_IMAGE::cuda::detail::delta
{
#if defined(_WIN32)
    constexpr static std::string_view s_plugin_name = "compressed_delta_plugin.dll";
#elif defined(__linux__)
    constexpr static std::string_view s_plugin_name = "libcompressed_delta_plugin.so";
#else
...
NAMESPACE_COMPRESSED_IMAGE
{
    namespace cuda
    {
        namespace filter
        {
            /// \brief delta filter plugin for CUDA compression.
            ///
            ...
            struct delta
            {
                ...
                {
                    return cuda::enums::filter::delta;
                }
                ...
                cudaStream_t stream) const
            {
                ...
                plugin_call(forward_fn_, "run_delta_forward", d_input, d_output, length, typesize, row_stride, stream);
            }

            inline void delta::backward(
                const uint8_t* d_input,
                uint8_t* d_output,
                size_t length,
                size_t typesize,
            ...
            {
                _COMPRESSED_PROFILE_FUNCTION();
                plugin_call(backward_fn_, "run_delta_backward", d_input, d_output, length, typesize, row_stride, stream);
            }

            template <typename Func, typename... Args>
            void delta::plugin_call(Func func, std::string_view func_name, Args&&... args)
            {
                if (!func)
                {
            ...
            inline delta::delta()
            {
                handle_ = proc::load_library(std::string(detail::delta::s_plugin_name));
                if (!handle_)
                {
                    return;
                }
