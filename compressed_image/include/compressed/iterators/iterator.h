            ...
            if (this->decompression_buffer_byte_size() % sizeof(T) != 0)
            {
                throw std::runtime_error(
                    std::format(
                        ...
                        sizeof(T),
                        decompression_buffer_byte_size()
                    )
                );
            }

            std::span<T> item_span(
                reinterpret_cast<T*>(m_DecompressionBuffer.data()),
                m_DecompressionBufferSize / sizeof(T)
            );
        ...
        bool valid() const
        {
            ...
            return std::visit(
                [&](auto& schunk)
                {
                    ...
                    bool compressed_data_valid = compression_buffer_max_byte_size() >= blosc2::min_compressed_size(
                        this->chunk_bytes()
                    );
                    bool decompressed_data_valid = decompression_buffer_max_byte_size() >=
                        blosc2::min_decompressed_size(this->chunk_bytes());

                    return idx_valid && compressed_data_valid && decompressed_data_valid && compression_size_valid &&
